...
defmodule StripeTest.ApiResourceTest do
  ...
  describe ".save_nested_resource" do
    test "can have a scalar set" do
    ...
    test "set a flag if given an object source" do
      # r = NestedTestAPIResource.new("test_resource")
      # r.external_account = %{
      #   object: "card",
      # }
  ...
  test "using a nil api key should raise an exception" do
  ...
  test "specifying api credentials containing whitespace should raise an exception" do
    # Stripe.api_key = "key "
    # assert_raises Stripe.AuthenticationError do
    #   Stripe.Customer.new("cus_123").refresh
    # end
  ...
  test "send expand on fetch properly" do
  ...
  test "preserve expand across refreshes" do
    # stub_request(:get, "#{Stripe.api_base}/v1/charges/ch_123")
    #   .with(query: %{ "expand" => ["customer"] })
    #   .to_return(body: JSON.generate(charge_fixture))
    #
  ...
  test "send expand when fetching through ListObject" do
  ...
  describe "when specifying per-object credentials" do
    describe "with no global API key set" do
      test "use the per-object credential when creating" do
    ...
    describe "with a global API key set" do
      setup do
      ...
      test "use the per-object credential when retrieving and making other calls" do
        # stub_request(:get, "#{Stripe.api_base}/v1/charges/ch_123")
        #   .with(headers: %{ "Authorization" => "Bearer sk_test_local" })
        #   .to_return(body: JSON.generate(charge_fixture))
  ...
  describe "with valid credentials" do
    test "urlencode values in GET params" do
    ...
    test "construct URL properly with base query parameters" do
      # stub_request(:get, "#{Stripe.api_base}/v1/charges")
    ...
    test "setting a nil value for a param should exclude that param from the request" do
      # stub_request(:get, "#{Stripe.api_base}/v1/charges")
    ...
    test "not trigger a warning if a known opt, such as idempotency_key, is in opts" do
      # stub_request(:post, "#{Stripe.api_base}/v1/charges")
      #   .to_return(body: JSON.generate(charge_fixture))
      # old_stderr = $stderr
      # $stderr = StringIO.new
    ...
    test "trigger a warning if a known opt, such as idempotency_key, is in params" do
      # stub_request(:post, "#{Stripe.api_base}/v1/charges")
      #   .to_return(body: JSON.generate(charge_fixture))
      # old_stderr = $stderr
      # $stderr = StringIO.new
    ...
    test "requesting with no ID should result in an InvalidRequestError with no request" do
      # c = Stripe.Customer.new
      # assert_raises(Stripe.InvalidRequestError) %{ c.refresh }
      needs_refactoring_into_elixir()
    end

    test "making a GET request with parameters should have a query string and no body" do
      # stub_request(:get, "#{Stripe.api_base}/v1/charges")
      #   .with(query: %{ limit: 1 })
      #   .to_return(body: JSON.generate(data: [charge_fixture]))
    ...
    test "making a POST request with parameters should have a body and no query string" do
      # stub_request(:post, "#{Stripe.api_base}/v1/charges")
    ...
    test "updating should fail if api_key is overwritten with nil" do
      # c = Stripe.Customer.new
      # assert_raises TypeError do
      #   c.save({}, api_key: nil)
      # end
    ...
    test "updating should use the supplied api_key" do
    ...
    test "passing in a stripe_account header should pass it through on call" do
      # stub_request(:get, "#{Stripe.api_base}/v1/customers/cus_123")
    ...
    test "passing in a stripe_account header should pass it through on save" do
      # stub_request(:get, "#{Stripe.api_base}/v1/customers/cus_123")
      #   .with(headers: %{ "Stripe-Account" => "acct_123" })
      #   .to_return(body: JSON.generate(customer_fixture))
    ...
    test "add key to nested objects" do
    ...
    test "save nothing if nothing changes" do
    ...
    test "not save nested API resources" do
    ...
    test "correctly handle replaced nested objects" do
      # acct = Stripe.Account.construct_from(id: "myid",
      #                                       legal_entity: %{
      #                                         last_name: "Smith",
      #                                         address: %{
    ...
    test "correctly handle array setting" do
      # acct = Stripe.Account.construct_from(id: "myid",
      #                                       legal_entity: %{})
    ...
    test "correctly handle array insertion" do
      # acct = Stripe.Account.construct_from(id: "myid",
      #                                       legal_entity: %{
      #                                         additional_owners: [],
      #                                       })
    ...
    test "correctly handle array updates" do
      # acct = Stripe.Account.construct_from(id: "myid",
      #                                       legal_entity: %{
    ...
    test "correctly handle array noops" do
      # acct = Stripe.Account.construct_from(id: "myid",
      #                                       legal_entity: %{
    ...
    test "correctly handle hash noops" do
      # acct = Stripe.Account.construct_from(id: "myid",
      #                                       legal_entity: %{
    ...
    test "should create a new resource when an object without an id is saved" do
    ...
    test "set attributes as part of save" do
      # account = Stripe.Account.construct_from(id: nil,
      #                                           display_name: nil)
