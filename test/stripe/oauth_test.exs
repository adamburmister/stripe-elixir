...
defmodule StripeTest.OAuthTest do
  ...
  describe ".authorize_url" do
    test "return the authorize URL" do
      uri_str =
        OAuth.authorize_url(
          scope: "read_write",
          state: "csrf_token",
  ...
  describe ".token" do
    test "exchange a code for an access token" do
      # The OpenAPI fixtures don't cover the OAuth endpoints, so we just
      # stub the request manually.
      stub_request(:post, "#{Stripe.connect_base()}/oauth/token").with(
        body: %{
  ...
  describe ".deauthorize" do
    test "deauthorize an account" do
      # The OpenAPI fixtures don't cover the OAuth endpoints, so we just
      # stub the request manually.
      stub_request(:post, "#{Stripe.connect_base()}/oauth/deauthorize").with(
        body: %{
          "client_id" => "ca_test",
